  ...
  for (let i = 0; i < this.balls.length; i++) {
   ...
   for (let j = i + 1; j < this.balls.length; j++) {
    ...
    if (distance < minDistance) {
     ...
     // Use when implementing more realism
     // const restitution = (ballA.restitution + ballB.restitution) / 2;
     const restitution = 1;
     const impulse = (-(1 + restitution) * velocityAlongNormal) / 2;
     ballA.velocityX += impulse * normalX;
     ballA.velocityY += impulse * normalY;
     ballB.velocityX -= impulse * normalX;
     ballB.velocityY -= impulse * normalY;
